...
(defn suborder
  "Calculates the order, i.e. size, of a subgroup of a cyclic group of 
  order m.  The original group is generated by an element a, and the subgroup
  is generated by an element a^k of the original group if it's a multiplicative
  group, or by ka if it's additive.  Specifically, returns m/gcd(k,m), as
  specified by Lidl & Niederreiter _Finite Fields_ Theorem 1.15(ii), p. 7."
  [m k]
  (/ m (nt/gcd m k)))
